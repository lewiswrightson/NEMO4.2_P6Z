...
CONTAINS
   ...
   SUBROUTINE p4z_ini( Kmm )
      ...
      DO jn = 1, jptra
        ...
        IF( cltra == 'CDZ'      )   jpcdz = jn      !: Diazotroph C biomass
        IF( cltra == 'FED'      )   jpfed = jn      !: Diazotroph Fe biomass
        IF( cltra == 'CHD'      )   jpchd = jn      !: Diazotroph Chl biomass
        IF( cltra == 'NDZ'      )   jpndz = jn      !: Diazotroph N biomass
        IF( cltra == 'PDZ'      )   jppdz = jn      !: Diazotroph P biomass 
      END DO
      ...
      IF( .NOT.ln_rsttr ) THEN  
         ...
         IF (ln_p6z ) THEN
            tr(:,:,:,jpcdz,Kmm) = bioma0
            tr(:,:,:,jpfed,Kmm) = bioma0 * 5.e-6
            tr(:,:,:,jpchd,Kmm) = bioma0 * 12. / 55.
            tr(:,:,:,jpndz,Kmm) = bioma0
            tr(:,:,:,jppdz,Kmm) = bioma0
         ENDIF
