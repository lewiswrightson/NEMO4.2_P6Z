   ...
   SUBROUTINE p4z_opt( kt, knt, Kbb, Kmm )
      ...
      DO_3D( 0, 0, 0, 0, 1, jpkm1)
         ...
         ELSE
            ...
            IF( ln_p5z .OR. ln_p6z  )       zchl = zchl + tr(ji,jj,jk,jppch,Kbb) * 1.e6
         ENDIF
      ...
      ENDIF
      IF( ln_p6z) THEN
         ! Explicit Diazotroph when using PISCES-QUOTA (P6Z)
         ALLOCATE( zetmp5(A2D(0)) )  ;   zetmp5 (:,:) = 0.e0
         DO_3D( 0, 0, 0, 0, 1, nksr)
            IF( gdepw(ji,jj,jk+1,Kmm) <= MIN(hmld(ji,jj), heup_01(ji,jj)) ) THEN
               zetmp5(ji,jj)  = zetmp5 (ji,jj) + ediaz(ji,jj,jk) * e3t(ji,jj,jk,Kmm)
            ENDIF
         END_3D
         !
         ediazm(:,:,:) = ediaz(:,:,:)
         ...
         DO_3D( 0, 0, 0, 0, 1, nksr)
            IF( gdepw(ji,jj,jk+1,Kmm) <= hmld(ji,jj) ) THEN
               z1_dep = 1. / ( zdepmoy(ji,jj) + rtrn )
               ediazm(ji,jj,jk) = zetmp5(ji,jj) * z1_dep
            ENDIF
         END_3D
         DEALLOCATE( zetmp5 )
      ENDIF
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nampisopt in configuration namelist' )
      ...
      IF( ln_p5z .OR. ln_p6z  )  epico    (:,:,:) = 0._wp
