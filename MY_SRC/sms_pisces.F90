MODULE sms_pisces   
   ...
   ! Explicit Diazotroph
   REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:,:) ::   sizedz     !: size of diazotrophs
...
CONTAINS
   ...
   INTEGER FUNCTION sms_pisces_alloc()
      ...
      INTEGER ::   ierr(20)        ! Local variables
      !!----------------------------------------------------------------------
      ierr(:) = 0
      ...
      IF( ln_p6z ) THEN
         ...
         ALLOCATE(  sizedz(A2D(0),jpk), sizedza(A2D(0),jpk), STAT=ierr(19) )
         !* Maximum nutrient uptake by diazotroph
         ALLOCATE( vnfmax_dz(A2D(0),jpk),  STAT=ierr(20) )
      ENDIF       
